...
namespace CTE
{
	...
	void CTE_SwapChain::cleanupSyncObjects()
	{
	...
	VkResult CTE_SwapChain::acquireNextImage(uint32_t* imageIndex)
	{
		vkWaitForFences( _device.device(), 
			1, 
	...
	VkResult CTE_SwapChain::submitCommandBuffers(const VkCommandBuffer* buffers, uint32_t* imageIndex) {
	...
	void CTE_SwapChain::createSwapChain() 
	{
	...
	void CTE_SwapChain::recreateSwapChain()
	{
	...
	void CTE_SwapChain::createImageViews() 
	{
		...
		for (size_t i = 0; i < swapChainImages.size(); i++) 
		{
			...
			if (vkCreateImageView(_device.device(), &viewInfo, nullptr, &swapChainImageViews[i]) !=
				VK_SUCCESS) 
			{
	...
	void CTE_SwapChain::cleanupSwapChain()
	{
		...
		for (int i = 0; i < depthImages.size(); i++) {
			vkDestroyImageView(_device.device(), depthImageViews[i], nullptr);
			vkDestroyImage(_device.device(), depthImages[i], nullptr);
			vkFreeMemory(_device.device(), depthImageMemorys[i], nullptr);
		}
		...
		vkDestroyRenderPass(_device.device(), renderPass, nullptr);
	}

	void CTE_SwapChain::createRenderPass()
	{
	...
	void CTE_SwapChain::createFramebuffers()
	{
	...
	void CTE_SwapChain::createDepthResources()
	{
		...
		for (int i = 0; i < depthImages.size(); i++)
		{
			...
			if (vkCreateImageView(_device.device(), &viewInfo, nullptr, &depthImageViews[i]) !=
				VK_SUCCESS) 
			{
				throw std::runtime_error("failed to create texture image view!");
			}
	...
	void CTE_SwapChain::createSyncObjects() 
	{
	...
	VkSurfaceFormatKHR CTE_SwapChain::chooseSwapSurfaceFormat( const std::vector<VkSurfaceFormatKHR>& availableFormats)
	{
	...
	VkPresentModeKHR CTE_SwapChain::chooseSwapPresentMode(
	...
	VkExtent2D CTE_SwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities)
	{
	...
	VkFormat CTE_SwapChain::findDepthFormat() 
	{
